...
Status: **ALL 17 STAGES COMPLETE (PLUS 13b), 2026-09-17 — THE FULL 196-SECTION PLAN IS BUILT** (audit §A; address infra §F; quote engine §G; checkout integration/payment trigger §H; state machine §I; seller fulfillment hook §J; rider dispatch §K; routes/pickup §L; PIN confirmation §M; customer tracking §N; admin operations dashboard §O; notifications §P; homepage widget §Q; batch offers §R; delivery feedback §S; analytics §T). Stage 4 (bundle engine) is a confirmed no-op by design (§B item 2) — the only stage intentionally not built, and that decision was the founder's own, not a gap. Two real gaps were found mid-session and fixed rather than left out: Stage 13b (delivery feedback, §E.1 Patch 3) had been skipped while working through 12/13/14/15/16 in a different order, and a prerequisite admin-RLS gap was found and fixed before Stage 16's UI could even be built (§O). The full pipeline runs end-to-end from payment through dispatch (including batch-onto-an-existing-route offers), pickup, PIN-confirmed delivery, automatic payout computation, customer-facing live tracking (both a dedicated page and a homepage widget), full admin visibility/control/analytics, customer email notifications at every meaningful status change, and post-delivery compliment/complaint collection surfaced to sellers and admin, with no dead end anywhere. Everything built is verified — real transactional tests directly in the database (full order→paid→dispatch→offer→accept→route→pickup→delivery-leg→arrival→PIN-confirm→payout chain, exercised end-to-end more than once, plus tracking/admin RPC ownership and staleness logic, plus pricing-version-immutability, plus notification dedup, plus the homepage widget's own delivery query, plus a full batch-offer scenario mirrored at the SQL level, plus feedback resolution/RLS, plus analytics arithmetic checked against constructed test data), logic tests of the JS involved, and repeated live security-advisor sweeps — not just code review. **Nothing is visibly live to real customers yet**: pricing is still gated on `GOOGLE_ROUTES_SERVER_KEY` (§C). Isqalo's real pickup address is now set (founder-provided, §C). Founder has also flagged Photon's address-suggestion accuracy as not good enough for production — tracked as a concrete future task (server-side Google Places proxy, §C item 4), continuing with Photon for now. Real security/access gaps have been caught and fixed at nearly every stage (see §K, §L, §O) — this is a pattern worth noting, not just a one-off: **every stage's own end-to-end verification found something the previous review pass missed.** Stages 11-14, 16, and 17 were all clean on their advisor sweep first pass; Stage 15 needed no new migration at all. One testing-methodology mistake caught and fixed mid-session (§S): a cross-call `set_config` reuse gave a false-pass RLS result once, caught by checking the actual row and re-verified correctly within a single transaction. Known gaps still tracked, not forgotten, all in the same category: no local Netlify dev environment was available this session, so every frontend piece (`track.html`, all new admin tabs, the homepage widget, the batch-offer HTTP path, Stage 13b's three surfaces) is verified via syntax checks + real database-level testing, not live-browser/live-request testing — `drivers` has 0 rows in production right now so nothing could be visually smoke-tested against real data either. Stage 14's email send and Stage 17's analytics numbers are similarly unvalidated against real production traffic (there isn't any yet). Stage 11's ETA-impact threshold remains a distance-based placeholder pending the founder's real number (§C item 5). **What's left is entirely founder-side now** — see §C's remaining action items (Google Routes key, batching threshold, payout formula, support contact info) and actually watching the system run once real delivery volume exists.
...
1. **One Google API key — server-side only, no browser key needed at all** (superseded 2026-09-17, see §B item 8). **Status: still not provided — this is now the ONLY remaining code-blocking item on this entire list.** Everything that depends on it (address autocomplete, distance/duration pricing, batch-offer time estimates) is fully built and wired to `process.env.GOOGLE_ROUTES_SERVER_KEY`; it just needs the real key added in Netlify. Needed APIs in Google Cloud Console: **Routes API** and **Places API (New)** (added 2026-09-17 once the founder confirmed the Photon→Google address-autocomplete switch, item 4 below) — restrict the key to just those two, nothing else, and set a billing quota/alert.
2. ~~Isqalo Shisanyama's real pickup address~~ — **RESOLVED 2026-09-17.** Founder set the real address via `seller-dashboard.html`'s pickup UI (`sellers.pickup_geo` now `POINT(31.0269074 -29.8520038)`).
3. **Initial service-zone boundary** for the Durban Workshop area (real polygon, or an accepted-as-temporary radius circle around Isqalo). Needed for Stage 2. **Status: not provided — will use a placeholder radius circle until real boundaries are given (can also be drawn later via the admin service-zone editor, which exists — see §F).**
4. ~~Photon address-suggestion accuracy~~ — **RESOLVED 2026-09-17.** Founder confirmed switching to Google. Built exactly as designed when this was first flagged: no browser-facing Google key, a new **server-side Google Places autocomplete proxy** (`netlify/functions/google-places.js`, reuses the same `GOOGLE_ROUTES_SERVER_KEY`) called from the browser instead of hitting Google directly. `checkout.html`, `profile.html`, and `seller-dashboard.html`'s address autocomplete all switched from OpenStreetMap Photon to this proxy, **with an automatic Photon fallback**: Google is tried first, and only on failure (key not yet added, network error, anything) does the exact old Photon call run instead — this was a deliberate call given checkout is a live, payment-adjacent flow, and a hard cutover would have meant zero address suggestions anywhere in the app until the key is added. **Blocked only on item 1 above** to actually prefer Google; until then, every address field keeps working exactly as it does today via the Photon fallback, and switches over to Google automatically the moment the key lands in Netlify — no further code changes needed either way.
5. ~~Confirmation that the founder's and partner's existing rep/`Logistics` accounts are the same two people to seed as `drivers` rows~~ — **RESOLVED earlier in this build** ("same accounts for now" — founder's own words). `drivers` self-registers on first heartbeat from those same accounts, no separate onboarding needed.
6. ~~Batching ETA-impact threshold~~ — **RESOLVED 2026-09-17.** Founder gave real numbers: **90 minutes max, ~60 minutes average target.** Built into `netlify/functions/lib/batch-dispatch.js`'s `MAX_BATCH_IMPACT_MINUTES`/`TARGET_AVERAGE_BATCH_IMPACT_MINUTES` — see §U for how the added-minutes estimate is computed without needing a live Google Routes call per batch evaluation.
7. ~~Initial driver payout formula~~ — **RESOLVED 2026-09-17.** Founder gave the real formula (base + per-km + per-extra-drop + per-extra-pickup + per-minute-over-free-time, floored at a minimum) with real Rand values. Built into the `compute_driver_payout_on_route_completion` trigger (authoritative, uses the route's real final stats) and `netlify/functions/lib/payout-formula.js` (the pre-acceptance estimate shown to a driver before they accept an offer) — see §U.
8. ~~Support contact info~~ — **RESOLVED 2026-09-17.** Founder gave the number: `+27797662768` (WhatsApp or call). Already used in `netlify/functions/lib/notify.js`'s FAILED-delivery email; now also added as a "Need help?" entry point on `track.html` — see §U.
...
## M. Stage 13 — PIN delivery confirmation (2026-09-17)

Chosen as the next stage over Stage 11/12 because Stage 10 deliberately dead-ends at `ARRIVING`/`PIN_REQUIRED` with no way to actually finish a delivery — this was the single blocking gap in the whole pipeline. Built entirely by me, no subagent (payment-adjacent: this is the function that fires the Stage 10 payout trigger).

**Migration:** `delivery_stage13_pin_confirmation` — adds `deliveries.pin_attempt_count` and `deliveries.pin_locked_at`. No new tables (per CLAUDE.md — reused the existing `deliveries`/`delivery_events`/`route_stops`/`routes` tables throughout).

**Files:**
- `netlify/functions/confirm-delivery-pin.js` (new) — the only place in the codebase permitted to set `deliveries.status = 'DELIVERED'`, enforcing design principle #10 ("no PIN, no completed delivery") literally, not as a suggestion. Verifies the caller is the assigned driver via `route_stops → routes → drivers` (never trusts a client-supplied driver/route id). A wrong PIN never changes delivery status — it stays `PIN_REQUIRED`. After 5 wrong attempts the delivery locks (`pin_locked_at`) and further attempts are refused with 423, going beyond the plan's own literal text (§163 only asks for logging failed attempts) because an unlimited-attempt 4-digit PIN is a real, cheap-to-close security gap. On a correct match: transitions the delivery to `DELIVERED` via `transitionDelivery()`, marks the drop `route_stops` row completed, and — generalizing correctly to a future multi-stop route rather than hardcoding "the only stop" — checks whether *all* of the route's stops are now completed, and if so marks the route completed too, which fires Stage 10's `compute_driver_payout_on_route_completion` trigger.
- `logistics.html`: replaced the Stage 10 "PIN confirmation isn't live yet" placeholder in the `arriving` branch of `renderActiveRoute()` with a real 4-digit PIN entry form wired to the new function (Bearer-auth POST, digit-only input sanitisation, inline error display, 423-lockout message passthrough, "Delivered ✓" success state, auto-refresh via `refreshDeliveryPanel()` after 1.5s). Diff is scoped exactly to that one branch — nothing else in the file touched.

**Known, explicitly tracked limitation:** no admin UI yet to unlock a `pin_locked_at` delivery (that's Stage 16). A lock is recorded as a `PIN_LOCKED` delivery_events row so Stage 16 has the data to build against; until then, unlocking means a direct DB update. Acceptable for a 2-rider pilot.

**Verified in the database** (real transactional test, not just code review): delivery creation with a real PIN → first PIN attempt correctly enters `PIN_REQUIRED` → 4 wrong attempts confirmed to leave status at `PIN_REQUIRED` (not locked) → 5th wrong attempt confirmed to set `pin_locked_at` and refuse further attempts (423) → after simulating an admin unlock, a correct PIN confirmed to transition to `DELIVERED`, complete the drop stop, auto-complete the route (last remaining stop), and auto-compute the correct `driver_payouts` row via the Stage 10 trigger. All test data cleaned up, zero leftover rows.

**Security advisor sweep after this stage:** clean relative to Stage 13 — the 5 SECURITY DEFINER/anon-callable findings and 3 RLS-no-policy findings in the current report all predate this stage (Stage 2's `find_service_zone_for_point`, and pre-existing seller/marketing tables unrelated to delivery) and are not regressions introduced here. `confirm-delivery-pin.js` is a Netlify function (service-role, not a Postgres RPC), so it doesn't surface in this scan at all — its access control is enforced entirely in the function body (bearer token → `auth.getUser` → `drivers` row → route ownership chain).

**Verified NOT touched:** no PayFast file, `checkout.html`, `validate-cart.js`, `seller-dashboard.html`, or `admin.html`.

---

## N. Stage 12 — customer tracking (2026-09-17)

Built entirely by me, no subagent. Chosen next because Stage 13 closed the last hard blocker in the pipeline, and this is the customer-facing payoff of everything built so far — plus the founder gave an explicit technical directive for it early in this build (no in-page GPS turn-by-turn, defer to a real map only; show "last seen Xm ago" once the rider's position is more than 90s stale, rather than pretending a stale fix is live).

**A pre-existing forward-declared gap closed, not just a new feature:** `deliveries.tracking_enabled` was added all the way back in Stage 5, defaulting to `false`, with nothing ever setting it — a column with no consumer. `create_delivery_on_payment()` (the Stage 5 trigger) now sets it `true` on every delivery's creation; this is the first real consumer.

**Migration:** `delivery_stage12_customer_tracking` (+ one follow-up fix, `delivery_stage12_fix_pin_cast`, for a `character(4)` vs `text` return-type mismatch caught immediately by a failing test, not left in). Adds `get_delivery_tracking(p_order_id uuid)` — a narrow `SECURITY DEFINER` RPC, deliberately not a broad new RLS policy on `drivers`: the `drivers` table has no customer-facing read policy today, and adding one would hand every tracking customer the assigned driver's `user_id`/`eligibility_status`/etc. The RPC verifies `deliveries.customer_id = auth.uid()` and `tracking_enabled = true` itself, then returns only what a customer should see: delivery status, their own delivery PIN (already customer-visible today via the existing `deliveries_select_own` RLS policy — not a new exposure), destination coordinates, the assigned driver's lat/lon + `last_location_at` (only once a driver is actually assigned, i.e. `route_id is not null`), and the pickup/drop stop statuses.

**Files:**
- `track.html` (new) — the tracking page itself. Requires a signed-in session (redirects to `index.html` otherwise, same convention as `profile.html`). Leaflet + OpenStreetMap tiles for the map (free, keyless — consistent with the founder's rejection of a browser-exposed Google Maps key and the existing Photon/OSM geocoding already in `checkout.html`), **not** a routing engine — no turn-by-turn, just a static pin for the destination and a marker for the rider's last known position, per the founder's explicit instruction. Polls `get_delivery_tracking` every 20s; stops polling once the delivery reaches `DELIVERED` or a terminal problem state (`CANCELLED`/`FAILED`/`RETURNED`). Implements the 90-second staleness rule literally: if `driver_location_at` is more than 90s old (the same `DRIVER_STALENESS_SECONDS` constant `dispatch.js` already uses for eligibility, reused here for consistency, not a second magic number), the marker greys out and the map note switches from "Showing your rider's live position" to "Rider location last seen Xm ago — may not be current" — never shown as if live when it isn't. Collapses the 16-state `delivery_status` enum into 5 customer-friendly stages (Order received / Rider assigned & collecting / On the way / Arriving / Delivered) plus a separate problem state, the same "translate the raw enum for customers" approach `profile.html`'s own `statusBadge()` already uses for `order_status`.
- `profile.html`: added a "📍 Track delivery" button to each order card's actions row, shown only when `orders.delivery_quote_id` is set (i.e. the order actually went through the new delivery pipeline) and the order isn't in a terminal cancelled/returned state. Diff scoped to exactly this addition — no other order-card logic touched.

**Verified in the database:** the RPC correctly returns nothing when the caller isn't the delivery's owner; correctly returns the delivery once `auth.uid()` matches `customer_id`; correctly decodes destination coordinates; correctly returns `null` driver position before a route/driver is assigned, and the real decoded driver lat/lon plus stop statuses once one is (confirmed against a driver location deliberately set 5 minutes stale, to exercise the >90s branch the UI keys off). All test data cleaned up, zero leftover rows. (One test-harness bug caught and understood, not a product bug: an early manual test appeared to show `driver_lat` never populating — traced to the test itself switching to the `authenticated` Postgres role *before* updating `deliveries.route_id`, which RLS then silently blocked since customers have no UPDATE policy on `deliveries`; re-run without that ordering mistake passed cleanly.)

**Verified NOT touched:** no PayFast file, `checkout.html`, `validate-cart.js`, `seller-dashboard.html`, `admin.html`, or `logistics.html`.

**Known gap, tracked not hidden:** no live-browser test was possible in this environment (no linked Netlify dev session with Supabase env vars available locally) — verification here is syntax-checking plus real database-level RPC testing, the same limitation and the same mitigation already used for every other stage's frontend work this session. Founder should click through `track.html` once a real delivery exists to confirm the map renders as expected.

---

## O. Stage 16 — admin operations dashboard (2026-09-17)

The plan's own §16 text was terse ("Full logistics view") but Fable's execution plan (§E) had a fuller breakdown into 6 independent units, explicitly flagged as "the strongest parallelization opportunity in the whole plan" since they touch disjoint `admin.html` sections and disjoint tables. Built that way: I fixed a security prerequisite and built the pricing editor myself, and dispatched two genuinely parallel subagents for the remaining units — the same pattern used successfully for Stage 8, and re-verified the same way (reading every diff hunk myself, re-testing every claim against the real database, not trusting subagent reports at face value).

**Prerequisite gap found and fixed before any UI work started:** `deliveries`, `delivery_events`, `delivery_quotes`, and `driver_offers` had **no admin RLS policy at all** — unlike `routes`/`route_stops`/`drivers`/`driver_payouts`/`delivery_pricing_config`, which already had the standard `*_admin_all` policy from earlier stages. Admins literally could not have read these tables from the browser before this. Fixed via `delivery_stage16_admin_rls`, mirroring the exact existing pattern (`EXISTS (SELECT 1 FROM admins WHERE admins.user_id = auth.uid())`), verified with a real RLS test (a non-admin authenticated user correctly sees nothing; an admin correctly sees and can write).

**Migrations:**
- `delivery_stage16_admin_rls` — the 4 missing admin policies above.
- `delivery_stage16_driver_locations_rpc` — new `list_online_driver_locations()`, `SECURITY INVOKER` (not `SECURITY DEFINER` — the caller's own RLS via `drivers_admin_all` already permits an admin to see every online driver, so no elevated-privilege function was needed), mirroring `get_route_stop_coordinates()`'s exact style for decoding a PostGIS geography column to lat/lon. Tested for real (inserted online/offline/null-location drivers, confirmed exactly the right one came back with correctly decoded coordinates, cleaned up).

**Files — `admin.html` only** (no PayFast/checkout/validate-cart/seller-dashboard file touched, confirmed via `git status` at close-out): three new sections added to the existing sidebar-nav/section-toggle pattern (`data-section="delivery-ops"`, `"deliveries"`, `"delivery-pricing"`), each lazy-loaded on first click, same convention as the existing Service Areas section.
- **Delivery Ops** (live map + riders roster) — built by a subagent. Leaflet + OpenStreetMap map (same free/keyless setup as `track.html`) showing every online driver, colored green/grey by the same 90-second staleness threshold used everywhere else in this build, via the new RPC. Riders roster is a plain table with an eligible⇄suspended toggle (direct RLS write, no new function needed) — deliberately no UI path to set `pending`, since that's an internal pre-approval state, not something to toggle back into.
- **Deliveries** (list + `delivery_events` timeline drill-in + exceptions) — built by a second, fully parallel subagent. The exceptions view is the acute gap this stage exists to close: PIN-locked deliveries (`pin_locked_at IS NOT NULL`) now get a real **"Unlock PIN"** admin action — clears `pin_locked_at`/`pin_attempt_count` and logs a `PIN_UNLOCKED` `delivery_events` row (never skips the audit log, this build's consistent convention for every state-changing action), closing the gap Stage 13 explicitly flagged and tracked. Also surfaces stale unaccepted `driver_offers` and `FAILED` deliveries (read-only visibility). The deliveries list is a manual-refresh table (not live-polling, correctly not trying to be `track.html`) with a click-to-expand full event timeline per delivery.
- **Delivery Pricing** (`delivery_pricing_config` CRUD) — built by me personally (fee/pricing-adjacent). Every save **inserts a new version row and only flips the previous active row's `is_active` to `false`** — existing version numbers and their numeric fields are never mutated, so a historical `delivery_quotes` row stays accurate against the exact tariff it was actually priced under. Verified for real: created a v2, confirmed v1's fields were byte-for-byte untouched and only its `is_active` flipped, confirmed exactly one active row at a time, then restored to the original single-version state (zero net change to production data).

**Subagent coordination note, not a problem:** two subagents and I were all told to work "near the Service Areas block" in the same file; the second subagent hit one stale-file `Edit` conflict from the first subagent's already-landed block, resolved it by re-reading and inserting after the other agent's own end-marker, and reported this proactively rather than silently working around it. I inserted my own (third) block last, after reading the fully-merged file, specifically to avoid the same collision — no functional overlap in the end, confirmed by reading every diff hunk.

**Verified:** all 2 new inline `<script>` blocks in `admin.html` syntax-check clean; full `git diff` read hunk-by-hunk (not just trusted); security advisor sweep clean relative to this stage (`list_online_driver_locations` doesn't even appear, correctly, since it's `SECURITY INVOKER`); real database tests for the RLS prerequisite, the driver-locations RPC, the PIN-unlock query/update/event-log path, and the pricing-versioning insert/deactivate logic — all cleaned up to original state, zero net data changes from testing.

**Known gap, tracked not hidden:** `drivers` currently has 0 rows in production (even the pilot's own riders aren't registered yet, since dispatch has never fired against a real order — pricing is still gated on `GOOGLE_ROUTES_SERVER_KEY`), so the live map and riders roster couldn't be visually smoke-tested against real data, only against synthetic inserted-then-cleaned-up rows. Same class of limitation already flagged for `track.html` in Stage 12 — no local Netlify dev environment available this session for live-browser testing. Founder should click through all three new admin tabs once real drivers/deliveries exist.

---

## P. Stage 14 — notification system (2026-09-17)

Built entirely by me (touches `lib/delivery-state.js`, a shared/central file used by every delivery-adjacent function, so kept out of subagent hands). Plan §168 explicitly asks for centralized triggering rather than scattering notification calls across dispatch/pickup/PIN functions — that's exactly what this does: one `notify(supabase, deliveryId, toStatus)` call added inside `transitionDelivery()` itself, so every future delivery-status-changing code path gets it for free, forever, with zero additional wiring.

**Scope decision, made explicitly, not silently narrowed:** the plan's §61 lists EMAIL and IN-APP as the two initial channels. This build ships **email only**. There is no existing customer-facing in-app notification feed anywhere in this codebase (`seller_notifications` is seller-facing only) — building a whole new in-app notification-center UI just to duplicate what `track.html` (Stage 12) already shows a customer the moment they open it isn't justified for a pilot with no real delivery volume yet. `notification_log.channel` is still a real, checked column (`'email'|'in_app'`) so an in-app channel slots in later without a schema change.

**Migration:** `delivery_stage14_notification_log` — new `notification_log` table (id, delivery_id, order_id, recipient_id, event_type, channel, status, provider_message_id, sent_at, created_at) plus a **unique index on `(coalesce(delivery_id, order_id), event_type, channel)`** for dedup (plan §111/169) — a customer can never get the same delivery-status email twice, even under a race. Admin-only RLS (`notification_log_admin_all`), matching every other log-style table in this build.

**Files:**
- `netlify/functions/lib/notify.js` (new) — the single `notify()` entry point. Only 5 of the delivery-status enum's 16 states actually email a customer (`IN_ROUTE`, `ARRIVING`, `DELIVERED`, `FAILED`, `CANCELLED`) — the plan itself says "not every event necessarily needs an email, some are in-app only," and the granular internal states (`ASSIGNED`, `PICKED_UP`, `NEXT_STOP`, etc.) are exactly that in-app-only case, already covered by `track.html`. Copy follows plan §58/59's tone guidance (warm/expressive, not "Order dispatched") — `IN_ROUTE` and `ARRIVING` both include the delivery PIN directly in the email body (plan §59's own example does this) and a link to `track.html?order=<id>`, tying directly back into Stage 12. Never blocks or fails the transition it's called from — every failure mode (no `RESEND_API_KEY`, no customer email on file, Resend API error) is caught, logged to `notification_log` as `status: 'failed'` where applicable, and swallowed.
- `netlify/functions/lib/delivery-state.js` — `transitionDelivery()` now `await`s `notify()` (not fire-and-forget: a Netlify Function's process can be frozen the instant its handler's promise resolves, so an un-awaited call risks never completing — matches this codebase's existing `dispatchDelivery()` call in `advance-delivery-on-fulfillment.js`, which is awaited but error-tolerant the same way).
- `netlify/functions/send-on-the-way.js` — **reconciled, not rewritten**, per the plan's own explicit instruction ("reconcile, don't duplicate... extend/wrap, don't rewrite"). This function's own trigger logic, auth check, rate-limiting, and rep-collection copy are **completely untouched**. Only its previously-inline HTML header/footer chrome (identical boilerplate to `notify.js`'s templates) now calls the new shared `emailShell()`/`esc()` helpers exported from `lib/notify.js`, instead of carrying its own separate copy — the direct CLAUDE.md "don't have files with duplicate copies of the same thing" fix the plan called out. **Verified byte-for-byte**: reconstructed the exact old inline template and the exact new `emailShell()`-based output side by side in a Node script and confirmed `newHtml === oldHtml` — this was a true no-op refactor of the live service-collection email path, not a behavior change.

**Verified:** `node --check` on all 3 touched files; `notify()` unit-tested directly (no-RESEND_API_KEY path and non-notifiable-event path both confirmed to skip cleanly without throwing, using a garbage `{}` Supabase client to guarantee no accidental real DB/network call on either fast-path); the `notification_log` dedup unique index tested for real against the database (identical `(delivery_id, event_type, channel)` correctly rejected as a `unique_violation`, a different `event_type` for the same delivery correctly allowed) — all test data cleaned up, zero leftover rows; `send-on-the-way.js`'s HTML output verified byte-identical before/after.

**Verified NOT touched:** no PayFast file, `checkout.html`, `validate-cart.js`, `seller-dashboard.html`, `admin.html`, or `logistics.html` — only `lib/notify.js` (new), `lib/delivery-state.js`, and `send-on-the-way.js`.

**Known gap, tracked not hidden:** `RESEND_API_KEY` is presumably set in production (this function reuses the exact same env var `send-on-the-way.js`/`send-referral-email.js` already depend on) but wasn't available to test an actual end-to-end email send in this session's sandbox — verification here is unit/DB-level, the same class of limitation already flagged for `track.html` and Stage 16's admin UI. Founder should trigger a real delivery through to `IN_ROUTE`/`DELIVERED` once live and confirm an actual email lands.

---

## Q. Stage 15 — homepage active-delivery widget (2026-09-17)

The plan's own §15 note says this is "substantially delivered by Stage 12/Unit D; kept as its own stage for tracking per original numbering" — cosmetic polish on `index.html`, per plan §50's exact state-by-state widget spec. Built entirely by me, small and self-contained.

**Files:** `index.html` (new `#activeDeliveryWidget` element, hidden by default, right at the top of `<main>` above the hero carousel so it's the first thing a customer with an active order sees; a `supabase-ready` listener querying `deliveries` for the signed-in customer's most recent row) and `style.css` (`.active-delivery-widget` — reuses the existing `var(--accent)`/`var(--accent-2))` gradient tokens already used elsewhere on this page, no new color invented).

**One deliberate, documented scope narrowing:** plan §50's "During active travel" example copy is "Arriving in about 7 minutes" — a literal ETA-minutes figure. This build has no ETA-recompute engine (the founder's own early instruction ruled out in-page navigation/routing machinery, and no live "minutes remaining" calculation exists anywhere in the delivery pipeline); showing a fabricated number would be dishonest, the same principle `track.html`'s "last seen Xm ago" staleness label already applies. Used status-driven copy instead ("Your order is on the way" for `IN_ROUTE`/`NEXT_STOP`, "Your driver has arrived" for `ARRIVING`/`PIN_REQUIRED`) — faithful to the state-by-state UX intent, not to the literal illustrative number.

**State mapping (§50 → this build's 16-state enum):** "being prepared" ← `PENDING`/`READY_FOR_DISPATCH`/`OFFERED`/`REASSIGNING`; "with a driver" ← `ASSIGNED`/`DRIVER_AT_PICKUP`/`PARTIALLY_PICKED_UP`/`PICKED_UP`; "on the way"/"arrived" ← the travelling/arriving states above; "delivered ✓" ← `DELIVERED`, shown for 2 hours after `delivered_at` then correctly stops appearing at all, matching §50's "disappears... no need to permanently occupy homepage real estate." `CANCELLED`/`FAILED`/`RETURNED` are not part of §50's widget states at all and are deliberately hidden rather than shown as an alarming homepage banner — a customer in one of those states already has visibility via `profile.html`/`track.html` if they go looking.

**Verified:** both files' inline scripts syntax-check clean; the widget's exact `deliveries` query (customer_id-scoped, ordered by `created_at desc`, limit 1) tested for real against the database as the owning customer, confirmed it finds a freshly created `PENDING` delivery — cleaned up, zero leftover rows. No new migration, no RLS change (reuses the existing `deliveries_select_own` policy).

**Verified NOT touched:** no PayFast file, `checkout.html`, `validate-cart.js`, `seller-dashboard.html`, `admin.html`, or `logistics.html` — only `index.html` and `style.css`.

**Known gap, tracked not hidden:** no local Netlify dev environment available this session to visually confirm the widget renders/positions correctly on a real page load — same class of limitation as every other frontend piece built this session. Founder should confirm visually once a real order reaches an active delivery state.

---

## R. Stage 11 — optional dynamic batch offers (2026-09-17)

Explicitly marked optional by the plan itself, and the founder's own standing instruction ("continue... as long as everything is implemented, nothing gets left out") is what pushed this from "skippable" to "build it anyway, just don't over-engineer it." Built entirely by me (touches the shared dispatch/accept code path used by every delivery). Deliberately kept to the plan's own pre-approved simplification: **append-to-end-of-current-stops only**, not the fuller §33 3-option route-insertion comparison — tracked as a named future enhancement once a driver regularly carries 3+ simultaneous stops.

**Founder input still pending, used as a placeholder, not invented silently:** §C item 5 already tracked "the batching ETA-impact threshold" as an open founder decision. There's no live routing/duration available internally to express this as true minutes (Google Routes is reserved for the paid customer-facing quote — same reasoning `dispatch.js` already documents for omitting `duration_impact_min`), so it's implemented as a straight-line-distance proxy (`PLACEHOLDER_BATCH_IMPACT_KM = 1.5`) until the founder gives a real number and `GOOGLE_ROUTES_SERVER_KEY` is live.

**Migration:** `delivery_stage11_batch_candidates_rpc` — new `find_batchable_routes(p_seller_geo, p_max_age_seconds)`, mirroring `find_nearest_eligible_drivers`'s exact pattern (`SECURITY DEFINER`, locked down from `anon`/`authenticated`/`public`): finds `'started'`/`'active'` routes whose driver's location is fresh (same 90s staleness constant used everywhere else), ordered nearest-first to the new pickup point.

**Files:**
- `netlify/functions/lib/batch-dispatch.js` (new) — `evaluateBatchCandidates()`, mirroring `lib/dispatch.js`'s `dispatchDelivery()` structure closely (same guard order: not-ready → already-offered → quote → multi-seller check → pickup location → candidate search → threshold check → offer insert → `transitionDelivery()` to `OFFERED`). Creates a `driver_offers` row with `offer_type: 'batch_addition'` and the existing route's `route_id` already attached.
- `netlify/functions/evaluate-batch-candidates.js` (new) — thin admin-only HTTP wrapper, mirroring `dispatch-delivery.js`'s exact structure, for a manual re-check (not required by any UI this stage).
- `netlify/functions/advance-delivery-on-fulfillment.js` — now tries `evaluateBatchCandidates()` **first** when a delivery becomes `READY_FOR_DISPATCH` (cheaper for a 2-driver fleet than always tying up a separate idle driver), only falling through to Stage 8's idle-driver `dispatchDelivery()` if no batchable route is close enough. Same non-blocking guarantee as the existing dispatch call.
- `netlify/functions/respond-to-driver-offer.js` — the accept path now branches on `offer_type`: `'batch_addition'` offers get a new `appendToExistingRoute()` (re-validates the route is still the driver's and still `'started'`/`'active'` before appending, since time may have passed since the offer was created; computes the next `seq_order` and inserts pickup+drop at the end) instead of `createRouteForAcceptedOffer()`'s new-route path. This closes a gap the file's own Stage 10 comment had explicitly flagged and left open: *"Once Stage 11 adds 'batch_addition' offers... this will need to branch on offer_type — not done here."*

**Verified in the database:** `find_batchable_routes()` tested for real (a `'started'` route with a fresh, nearby driver location is found with the correct decoded distance; a stale driver location and a non-`'started'`/`'active'` route are both correctly excluded). A full realistic scenario tested end to end at the SQL level (mirroring exactly what the JS constructs): an existing 2-stop route for delivery A, a second delivery B from the same seller, `find_batchable_routes` correctly identifying A's route as a candidate, the batch offer insert, and `appendToExistingRoute`'s stop-appending logic correctly computing `seq_order = 3, 4` and linking delivery B to the shared route — all cleaned up, zero leftover rows. (One test-setup mistake caught immediately, not a product bug: the first attempt used Johannesburg-area coordinates for a seller whose real `pickup_geo` is in Durban, so distance came back `NULL`/over-threshold; corrected to use the seller's actual coordinates and re-ran successfully.)

**Verified NOT touched:** no PayFast file, `checkout.html`, `validate-cart.js`, `seller-dashboard.html`, `admin.html`, `logistics.html`, or `track.html`.

**Known gap, tracked not hidden:** no local Netlify dev environment to exercise the real HTTP request/response path end to end (same limitation as every other backend piece this session) — verified via direct SQL mirroring the exact operations the JS performs, plus `node --check` syntax verification, not a live function invocation. The ETA-impact threshold remains a distance-based placeholder pending the founder's real number (§C item 5).

---

## S. Stage 13b — delivery feedback / compliments & complaints (2026-09-17)

The gap found and flagged while closing out Stage 11 (§R): this stage was added during the original plan reconciliation (§E.1 Patch 3, between Stage 13 and 14) but never actually built, having been missed while working through 12/13/14/15/16 in a different order than originally reconciled. Built now, in full, per its original spec — not silently left out once found. Built entirely by me (touches the shared feedback-submission path plus three separate frontend surfaces).

**Migration:** `delivery_stage13b_feedback` — new `delivery_feedback` table (`delivery_id`, `order_id`, `customer_id`, `feedback_type` check `'compliment'|'complaint'`, `category`, `message`, `seller_id`, `driver_id`, `created_at`), with a **unique index on `delivery_id`** (one feedback submission per delivery — the prompt disappears once submitted). RLS: customer can `SELECT` their own (so `track.html` can detect "already submitted" and not re-prompt), seller can `SELECT` rows where `seller_id` matches their own shop, admin has full access.

**One deliberate security tightening beyond the plan's literal wording, documented not silently done:** the plan's own RLS description says "customer can insert their own (matched by delivery_id ownership)." This build does **not** add a client-facing INSERT policy at all — submission goes exclusively through `netlify/functions/submit-delivery-feedback.js` (service role), which resolves `seller_id`/`driver_id` itself from the delivery's actual route rather than trusting client-supplied values. A raw client INSERT with only a delivery-ownership check would let a customer freely set any `seller_id`, misattributing a complaint to the wrong shop — closing that off before it could ever be a problem, not after.

**Files:**
- `netlify/functions/submit-delivery-feedback.js` (new) — verifies the caller owns the delivery, that it's actually `DELIVERED` (feedback is post-delivery only, per plan §97), resolves `seller_id` from the pickup `route_stops` row and `driver_id` from `routes.driver_id`, and inserts. Returns a friendly 409 on a duplicate submission (the unique index is the real guarantee).
- `track.html` — after a delivery reaches `DELIVERED`, a "How was your Umzila delivery? ♡ Loved it / Something went wrong" prompt (plan §97's exact copy), branching into a compliment free-text box or a complaint flow with the plan's §96 category buttons ("My order was late" / "...missing something" / "I couldn't meet the driver" / "Another delivery problem") plus free text. Checks for existing feedback first and shows a thank-you state instead of re-prompting.
- `seller-dashboard.html` — new "💬 Feedback" tab, a simple compliment/complaint list scoped to the seller's own shop (plan §76's "Feedback" mini-section), reusing the existing tab-panel/empty-state/spinner conventions already used by the Notifications tab.
- `admin.html` — folded into the existing Stage 16 "Deliveries" section (`loadDeliveriesFeedback()`, a new `#deliveriesFeedbackMount`) rather than a separate admin build, exactly as the plan's own note said to do ("natural fit inside Stage 16 Unit F"). Each feedback row links to "View delivery timeline," reusing the delivery list's existing click-to-expand `delivery_events` drill-in (plan §98/173's "admin can see the complete timeline" investigation flow) — scrolls to and opens the matching row if it's in the currently-loaded list, with an honest fallback message if it's older than the last 50 shown.

**Verified in the database:** the resolution logic (`route_stops`→`seller_id`, `routes`→`driver_id`) tested against a real route with matching pickup/drop stops; the one-per-delivery unique index tested for real (a duplicate insert correctly rejected); RLS tested properly this time within a single transaction after an earlier same-session test mistake was caught (see below) — an unrelated authenticated user correctly sees zero rows and a raw client INSERT is correctly rejected outright (no authenticated INSERT policy exists by design). All test data cleaned up, zero leftover rows.

**A testing-methodology mistake caught and fixed, not a product bug:** an initial RLS negative-test tried to reuse a `set_config('role', 'authenticated', ...)` call from a *previous, separate* tool call — those settings don't persist across separate database connections, so that particular check silently ran as the bypass-RLS service connection and gave a false pass. Caught immediately by checking the row actually existed afterward, and re-run correctly as a single self-contained transaction (`set_config` + the insert attempt + the assertion all in one `DO` block, the same pattern already used successfully for every other RLS test this session) — confirmed the real behavior is correct.

**Verified NOT touched:** no PayFast file, `checkout.html`, `validate-cart.js`, `logistics.html`, or the Delivery Ops/Delivery Pricing sections of `admin.html`.

**Known gap, tracked not hidden:** no local Netlify dev environment to visually confirm the track.html prompt, seller-dashboard.html tab, or admin.html feedback list render correctly — same class of limitation as every other frontend piece this session.

---

## T. Stage 17 — analytics / optimisation (2026-09-17)

The last stage. The plan itself frames this differently from every other stage: "needs real operational data from Stages 5–14 live to be meaningful... should be treated as explicitly post-pilot-validation, not part of the Isqalo launch checklist" and its own §187 MVP list already excludes "advanced analytics." Built anyway, structurally correct, per the founder's standing "nothing gets left out" instruction — but honestly: with `drivers` at 0 rows and pricing still gated on `GOOGLE_ROUTES_SERVER_KEY`, this will show near-empty numbers until real delivery volume exists. That's the expected, correct state today, not a bug to chase.

**Migration:** `delivery_stage17_analytics` — two Postgres views over existing tables (`route_profitability_v` per plan §100, `delivery_analytics_summary_v` per plan §99), no new tables, per this stage's own "prefer views" migration note. Neither view is directly queryable by any client role — both are cross-seller/cross-customer aggregates with no natural per-row RLS scoping, so (matching this build's consistent pattern for that situation, e.g. `get_delivery_tracking`) they're exposed only through two `SECURITY DEFINER` functions, `get_delivery_analytics_summary()` and `get_route_profitability(p_limit)`, that check `admins` membership internally and raise an exception otherwise.

**Deliberately incomplete, documented not hidden:** plan §99's full metric list includes average pickup time, average rider acceptance time, and average customer ETA accuracy — these need per-event timestamp diffing against `delivery_events` (e.g. `DRIVER_ASSIGNED` → `ORDER_PICKED_UP` for pickup time), a meaningfully bigger and more failure-prone piece of SQL than the rest of this stage, and with zero real events in production right now there's nothing to validate it against even if built. Left out, tracked here explicitly (also surfaced as a small note directly in the admin UI itself, not just in this doc) rather than shipped untested and unverifiable. Everything else in §99's list is built. §144's per-service-zone breakdown was also left out: deliveries/`delivery_quotes` don't currently retain which `service_zones` row they matched at quote time, so a meaningful zone breakdown would need a real schema addition (a `zone_id` FK) — not something to casually bolt on for the explicitly lowest-priority, no-data-yet stage of the whole build.

**Files:** `admin.html` — new "📈 Delivery Analytics" section (lazy-loaded, same nav/section-toggle convention as every other admin tab added this session): a KPI summary grid (total/delivered/failed/cancelled deliveries, avg delivery time, PIN completion rate, compliments/complaints counts, total rider payout, total delivery revenue) and a per-route profitability table (distance, duration, delivery count, customer revenue, priority revenue, driver payout, gross logistics revenue remaining — plan §100's exact "ROUTE #348" breakdown, in table form).

**Verified in the database:** both RPCs tested for real — a non-admin caller correctly gets rejected; an admin caller correctly sees the aggregates; a constructed test scenario (one delivered delivery worth R40 on a route with a R30 driver payout and one compliment) confirmed `customer_delivery_revenue = 40`, `driver_payout = 30`, `gross_logistics_revenue_remaining = 10`, `delivered_count >= 1`, `compliments_count >= 1` — exact arithmetic checked, not just "no error." All test data cleaned up, zero leftover rows. Security advisor sweep clean relative to this stage — both new functions correctly appear only in the authenticated-callable list, never anon, matching `get_delivery_tracking`'s established pattern.

**Verified NOT touched:** no PayFast file, `checkout.html`, `validate-cart.js`, `seller-dashboard.html`, `logistics.html`, `track.html`, or any other section of `admin.html`.

**Known gap, tracked not hidden:** no local Netlify dev environment to visually confirm the admin UI renders correctly, and — unlike every other stage this session — genuinely no real data exists yet to confirm the numbers are *meaningful*, only that the arithmetic is correct against constructed test data. This is expected and explicitly acknowledged by the plan itself, not a shortfall unique to this build.

---

**This closes out the full 196-section plan.** Every stage from 1 through 17 has been built, including the two gaps found mid-session (the missing Stage 13b, and the admin-RLS prerequisite found while starting Stage 16) — both fixed rather than left out, per the founder's own non-negotiable instruction. What remains is exclusively founder-side: the items in §C (a Google Routes server key, the real batching ETA threshold, the real driver payout formula, support contact info, and reviewing the several placeholder values now live in code), plus actually watching the system behave once real delivery volume exists to validate Stage 17's numbers against.

---

## U. Founder decisions applied — real payout formula, real batch threshold, support contact, Google address search (2026-09-17)

Immediately after Stage 17 closed out the plan, the founder answered every remaining §C item in one message. Applied here, same session, same rigor (real DB tests, security sweeps, byte-for-byte-style verification where relevant) as every stage before it.

**1. Real driver payout formula.** `payout = max(minimum, base + per_km*route_km + per_extra_drop*(drops-1) + per_extra_pickup*(pickups-1) + per_min_over*max(route_minutes-free_minutes,0))`, with `base=R4, per_km=R4, per_extra_drop=R10, per_extra_pickup=R10, minimum=R20, free_minutes=30, per_min_over=R1.50`. Replaces the Stage 8/10 placeholder (75% of quoted fee) in two places:
- **Authoritative**: the `compute_driver_payout_on_route_completion` Postgres trigger now computes `route_km` (summed straight-line PostGIS distance between consecutive `route_stops` in `seq_order` — same internal-heuristic convention as `find_nearest_eligible_drivers`/`find_batchable_routes`, not the paid Google Routes API) and `route_minutes` (real wall-clock elapsed time, `routes.started_at` to completion) from the route's actual final state, not an estimate. Also backfills `routes.total_distance_km`/`total_duration_min`, which makes Stage 17's `route_profitability_v` view meaningful once routes actually complete, as a side effect.
- **Pre-acceptance estimate**: new `netlify/functions/lib/payout-formula.js` exports the same formula/constants for showing a driver a realistic number *before* they accept an offer (the real route hasn't happened yet, so this estimates from the delivery's own `delivery_quotes.distance_km`/`duration_min`, already real Google-Routes-computed data). `lib/dispatch.js` (new-route offers: 0 extra stops) and `lib/batch-dispatch.js` (batch-addition offers: 1 extra drop + 1 extra pickup, since joining an existing route is by definition one more of each) both call it, replacing the old flat 75%-of-fee estimate.
- **Verified in the database**: two real completed-route scenarios — a ~2km/40min trip (expected ≈R26.98, computed R26.98 exactly) and a very short trip (expected to hit the R20 floor, computed exactly R20.00) — both confirmed the formula, the minimum-floor clamp, and the `routes` backfill. The JS estimate was separately unit-tested against the same formula (3 hand-computed cases, all matched). All test data cleaned up, zero leftover rows.

**2. Real batching ETA-impact threshold.** Founder gave **90 minutes max, ~60 minutes as a typical/average target**. Interpreted as one hard gate (90min) plus a logged-but-not-gating average target (two simultaneously-active hard thresholds would be operationally ambiguous about which one actually governs) — this interpretation is explicit here, not assumed silently. `lib/batch-dispatch.js` estimates the added minutes for a candidate batch WITHOUT a new per-evaluation Google Routes call (that stays reserved for the customer-facing quote, so batch evaluation — which fires automatically and often — doesn't add ongoing API cost): it decomposes the estimate into the unknown leg (driver's current position → new pickup, estimated from `find_batchable_routes`' straight-line distance at an assumed 25km/h urban speed) plus the known leg (new pickup → destination, which is the delivery's own **real** `delivery_quotes.duration_min` from Google Routes at quote time) — meaningfully more accurate than the old flat-distance placeholder since half of it is real routed data. `driver_offers.duration_impact_min` (previously always `NULL`, deliberately omitted per Stage 8's own comment) is now populated with this estimate.

**3. Support contact info.** `+27797662768` (WhatsApp or call) — already reused correctly in `notify.js`'s FAILED-delivery email; now added as a real "Need help?" entry point on `track.html` (plan §96), with a WhatsApp deep-link that pre-fills the order id for context and a `tel:` link.

**4. Photon → Google Places address autocomplete.** New `netlify/functions/google-places.js` — a server-side proxy (never a browser-facing key, consistent with the founder's earlier explicit rejection of that) using Google Places API (New): `?action=autocomplete` and `?action=details` (place_id → address components + lat/lon), reusing `GOOGLE_ROUTES_SERVER_KEY`. `checkout.html`, `profile.html`, and `seller-dashboard.html`'s address autocomplete all switched over — **with an automatic Photon fallback added after user consultation**: since `GOOGLE_ROUTES_SERVER_KEY` isn't live in Netlify yet and checkout is a real, payment-adjacent flow, a hard cutover would have meant zero address suggestions anywhere in the app the moment this shipped. Google is tried first; only on failure (not-yet-configured, network error, anything) does the exact original Photon call run instead, verified via a control-flow simulation test (Google-succeeds/Google-fails/aborted-request all behave correctly, including that an aborted request never wastefully falls through to Photon). `onSelect(props, geometry)` keeps the exact shape both Photon and the new Google path produce, so none of `applyAddressSuggestion()`/`applyProfileAddressSuggestion()`/`applyPickupAddressSuggestion()` needed to change at all — only the fetch layer underneath them did.

**Verified:** all 8 touched/new files (`checkout.html`, `profile.html`, `seller-dashboard.html`, `track.html`, `google-places.js`, `payout-formula.js`, `dispatch.js`, `batch-dispatch.js`) pass syntax checks. Security advisor sweep clean — no new findings (the payout trigger was already locked down and stayed locked down; no new RPC was added for the Google Places proxy, it's a plain Netlify function). No PayFast file touched.

**Known gap, tracked not hidden:** none of this can be live-tested end-to-end until `GOOGLE_ROUTES_SERVER_KEY` actually exists in Netlify — the Google Places response-shape handling in particular (Places API (New)'s nested `addressComponents`/`placePrediction` structure) is built faithfully from documented API shapes but has not been exercised against a real API response. Founder should do one real search on `checkout.html` once the key is live and confirm suggestions look right before relying on it.
