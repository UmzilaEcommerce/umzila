...
Status: **STAGES 1-8 COMPLETE, 2026-09-17** (audit §A; address infra §F; quote engine §G; checkout integration/payment trigger §H; state machine §I; seller fulfillment hook §J; rider dispatch §K). Stage 4 (bundle engine) is a confirmed no-op by design (§B item 2). Everything built so far is verified — real transactional tests directly in the database (including a full dispatch→offer→accept chain), logic tests of the JS involved, and a live security-advisor sweep — not just code review. **Nothing is visibly live to real customers yet**: the whole pricing pipeline is still gated on `GOOGLE_ROUTES_SERVER_KEY` (§C), which hasn't been provided. A placeholder pickup location was set for Isqalo (§C item 3b) so the dispatch chain could actually be tested end-to-end — **still needs your confirmation**. Two real security gaps were caught and fixed during Stage 8's own verification (see §K) — both introduced by this build, both closed before this commit. Next: Stage 9/10 (pickup checklist + routes/stops — note the original plan numbers these in the wrong dependency order, Stage 10's `route_stops` must exist before Stage 9's `route_stop_items` can reference it, already flagged in §E's reconciliation notes).
...
3b. **Isqalo's real pickup address — status update, 2026-09-17: a placeholder was set** (`sellers.pickup_geo` = the UKZN Westville campus coordinate already used as this build's Photon search-bias point), because Stage 8's own end-to-end test discovered it was still `NULL` and that blocks dispatch entirely (not just Stage 3 pricing, as originally scoped — the dispatch distance calculation needs it too). **This still needs your confirmation/correction** — either give me the real address so I can geocode it properly, or confirm the placeholder is close enough for the pilot and I'll leave it. Until then, real dispatch distance calculations will be measured from this stand-in point, not Isqalo's actual door.
