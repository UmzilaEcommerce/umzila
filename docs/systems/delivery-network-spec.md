...
Status: **PLANNING COMPLETE, STAGE 2 NOT YET STARTED.** Stage 1 (audit) is done (§A). Execution plan for Stages 2–17 is done (§E), reconciled against the full original plan. No implementation code written yet. Next: explicit founder sign-off on §C item 8 (three judgment-call decisions), then Stage 2 begins.
...
4. **Pricing tariff:** use the **existing, live tariff table** as the real numbers — `DELIVERY_CLASS_PRICES = {small: 12, medium: 22, large: 50}`, `PER_SELLER_FEE = 3`, `FREE_DELIVERY_THRESHOLD = 600`, `MAX_DELIVERY_FEE = 80`, `DEFAULT_UNITS_PER_TRIP = {small: 8, medium: 4, large: 2}`, `LARGE_OVERFLOW_FEE = 10` (from `checkout.html`/`validate-cart.js`, confirmed live in production today) — **not** the illustrative "R36 starting at 0-3km" table in plan §12, which the plan itself calls "working assumptions rather than a final tariff." Road-distance-tiered pricing (plan §11–§13) should be layered on top of/reconciled with this existing class-based model, not replace it outright, unless Fable's execution plan makes a reasoned case for full replacement (founder has given Fable free rein to identify and fix problems — see item 6 below).
...
1. **Google Maps API key(s), two of them.** A browser-restricted (HTTP referrer) key for Places Autocomplete on `checkout.html` — same trust class as the Supabase anon key already exposed via `get-client-config.js`. A private server-side key (Netlify env var only, never sent to frontend) for the Routes/Geocoding API calls in `netlify/functions/`. Needed for Stage 2/3. **Status: not provided — placeholder env var names `GOOGLE_MAPS_BROWSER_KEY`/`GOOGLE_MAPS_SERVER_KEY` will be used in code until real values are added in Netlify.**
...
3. **Initial service-zone boundary** for the Durban Workshop area (real polygon, or an accepted-as-temporary radius circle around Isqalo). Needed for Stage 2. **Status: not provided — will use a placeholder radius circle until real boundaries are given (can also be drawn later via the admin service-zone editor once built).**
...
8. **Three explicit sign-offs**, since these are the plan's highest-judgment-call decisions and deserve a deliberate yes rather than an assumed one:
   - (a) The **trigger-based PayFast integration** approach for Stage 5 (a Postgres trigger on `orders.order_status → 'paid'` creates the `deliveries` row — `payfast-itn.js`/`generate-payfast-signature.js`/`charge-payfast-token.js` are never edited). This is the single biggest CLAUDE.md-compliance decision in the whole plan.
   - (b) The **"layer distance pricing on top of the existing live tariff, price-neutral at launch"** approach for Stage 3 (rather than replacing the tariff outright).
   - (c) **Exposing a referrer-restricted Google browser key** via the same safe-proxy pattern `get-client-config.js` already uses for the Supabase anon key (Stage 2) — flagged because a literal reading of CLAUDE.md's "no Supabase keys in frontend" rule could be over-read as "no third-party keys at all," which isn't the intent, but deserves your explicit confirmation rather than an assumption.
...
**Files**
- `netlify/functions/geocode-address.js` (new) — server-side Place Details/Address Validation proxy.
- `netlify/functions/check-service-zone.js` (new) — `ST_Contains` lookup against `service_zones`, returns eligibility + zone_type + reason string (§20 wording).
- `checkout.html` — replace/augment free-text address fields with Places Autocomplete UI; store `destination_geo`/`place_id` on the pending order draft.
- `profile.html` — saved address + delivery instructions editing; PIN display area.
- `seller-dashboard.html` — pickup-address entry/confirmation UI (§9), writing `sellers.pickup_geo`/`pickup_place_id` via `geocode-address.js`.
...
- [ ] B: `geocode-address.js` + `check-service-zone.js` (can be written against Google's docs before keys arrive; blocked from going *live* until founder provides keys)
- [ ] C: `checkout.html` autocomplete UI (buildable against a mocked geocode response)
- [ ] D: `seller-dashboard.html` pickup-address UI
- [ ] E: `admin.html` service-zone editor shell

**Dependencies**: Unit A before B/D/E write real data. Google keys (founder input) block B going live. Full Stage 2 (zones + geocoding working end-to-end for Isqalo + one Durban zone) must finish before Stage 3.

**Founder inputs needed**
- Google Places/Maps **browser** key (referrer-restricted — safe to expose client-side, same trust class as the already-exposed Supabase anon key)
- Google Routes/Geocoding **server** key (private, Netlify env var only, never sent to frontend)
...
**Concerns/flags**
- A literal reading of CLAUDE.md's "do not put any supabase keys in the frontend" could be over-read as "no third-party keys in frontend either." A browser-restricted Google Maps key is industry-standard to expose (like the Supabase anon key already is, via `get-client-config.js`) — flagging this interpretation explicitly for founder sign-off rather than silently assuming it's fine.
...
**Migrations**
- New `delivery_pricing_config`: `id, version int, effective_from timestamptz, class_prices jsonb default '{"small":12,"medium":22,"large":50}', per_seller_fee numeric default 3, free_delivery_threshold numeric default 600, max_delivery_fee numeric default 80, default_units_per_trip jsonb default '{"small":8,"medium":4,"large":2}', large_overflow_fee numeric default 10, priority_fee numeric default 15, distance_tiers jsonb, is_active boolean default true, created_by uuid references auth.users, created_at`. Seed row v1 with distance_tiers = zero surcharge everywhere (price-neutral launch, per cross-cutting decision #6).
...
**Founder inputs needed**
- Confirmation of the "layer, don't replace" pricing decision (cross-cutting #6)
...
**Founder inputs needed**: explicit sign-off on the trigger-based integration approach (cross-cutting #7) — this is the plan's single highest-leverage CLAUDE.md-compliance decision and deserves an explicit yes, not an assumed one.
