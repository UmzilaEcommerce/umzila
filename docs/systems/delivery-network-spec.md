...
5. **Batching ETA-impact threshold** (Stage 11) — the max extra wait you're willing to impose on an existing customer to justify adding a nearby order to a rider's route (e.g. "+10 min max"). The plan deliberately leaves this as your call, not something to invent silently. **Status: Stage 11 is built and live with a placeholder — since there's no real routing/duration available internally (Google Routes is reserved for the customer-facing quote, same reasoning `dispatch.js` already documents), the threshold is currently expressed as straight-line distance (1.5km) rather than true ETA minutes, in `netlify/functions/lib/batch-dispatch.js`'s `PLACEHOLDER_BATCH_IMPACT_KM`. Once you give a real number, this should become a true minutes-based check the moment `GOOGLE_ROUTES_SERVER_KEY` is live.**
