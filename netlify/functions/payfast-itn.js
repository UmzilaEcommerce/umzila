...
exports.handler = async function(event, context) {
    ...
    try {
        ...
        if (pfData.payment_status === 'COMPLETE') {
            ...
            // "Mark paid + run every one-time post-payment task" — shared with
            // charge-payfast-token.js's saved-card flow (see lib/complete-order-payment.js
            // for why that flow needs the same logic from a different trigger point).
            await completeOrderPayment(supabase, {
                mPaymentId: pfData.m_payment_id,
                pfPaymentId: pfData.pf_payment_id,
                pfResponse: pfData,
                siteUrl: SITE_BASE_URL
            }).catch(e => console.error('ITN: completeOrderPayment error', e));
